...
def _score(ctx: Ctx, cfg: dict) -> float:
    name = ctx.clip.stem.replace("_cropped_trimmed", "")
    tr, _, s, r = track_field_identity(ctx.clip, ctx.packs, ctx.lock, frame_wh=ctx.wh, **cfg)
...
def main() -> None:
    ...
    names = list(ctxs)
    print(f"scoring {len(GRID)} configs x {len(names)} clips ...")
    matrix: dict[tuple[int, str], float] = {}
    for gi, cfg in enumerate(GRID):
        for n in names:
            matrix[(gi, n)] = _score(ctxs[n], cfg)
    ...
    for n in names:
        ...
        print(f"{n.split('_')[0]:>5}  {matrix[(gi, n)]:.3f}     {GRID[gi]}")
